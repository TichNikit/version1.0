...
async def delete_game(db: Annotated[Session, Depends(get_db)], game_id: int):
    ...
    db.execute(delete(UserGameFeedback).where(UserGameFeedback.game_id == game_id)).all()
    db.execute(delete(UserGameRating).where(UserGameRating.game_id == game_id)).all()
    db.commit()
